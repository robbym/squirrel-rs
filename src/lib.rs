...
type PrintFn = extern "C" fn(v: ffi::HSQUIRRELVM, len: usize, buf: *const c_char);
...
/// Print callback
extern "C" fn print_fn<P: Write, E: Write>(v: ffi::HSQUIRRELVM, len: usize, buf: *const c_char) {
...
/// Error callback
extern "C" fn err_fn<P: Write, E: Write>(v: ffi::HSQUIRRELVM, len: usize, buf: *const c_char) {
    let buffer = unsafe { slice::from_raw_parts(buf as *const u8, len) };
...
extern "C" fn err_handler(v: ffi::HSQUIRRELVM,
                          desc: *const ffi::SQChar,
                          source: *const ffi::SQChar,
                          line: ffi::SQInteger,
                          column: ffi::SQInteger) {
    unsafe {
...
extern "C" fn read_fn<C: Iterator<Item = char>>(ptr: ffi::SQUserPointer) -> ffi::SQInteger {
...
impl<P: Write + Sync, E: Write + Sync> SquirrelVM<P, E> {
    ...
    pub fn suspend(&mut self) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_suspendvm(self.0) }, (), ())
    }
    pub fn wake_up(&mut self,
                   ...
                   -> Result<(), ()> {
        get_result(unsafe {
                       ffi::sq_wakeupvm(self.0,
                                        resumed_return as ffi::SQBool,
                                        return_value as ffi::SQBool,
                                        raise_error as ffi::SQBool,
                                        throw_error as ffi::SQBool)
                   },
                   (),
                   ())
    }
    ...
    pub fn compile<C: Iterator<Item = char>>(&mut self,
                                             chars: &mut C,
                                             name: &str)
                                             -> Result<(), CompilerError> {
        let name = CString::new(name).unwrap();
        let result = unsafe {
            ffi::sq_compile(self.0,
                            read_fn::<C>,
                            mem::transmute(chars),
        ...
        if ffi::SQ_SUCCEEDED(result) {
            ...
            let data: &mut Option<CompilerError> = unsafe {
                mem::transmute(ffi::sq_getforeignptr(self.0))
            };
    ...
    pub fn push(&mut self, idx: isize) {
        unsafe {
            ffi::sq_push(self.0, idx);
        }
    }

    pub fn pop(&mut self, element_count: isize) {
        unsafe {
            ffi::sq_pop(self.0, element_count);
        }
    }

    pub fn pop_top(&mut self) {
        unsafe {
            ffi::sq_poptop(self.0);
        }
    }

    pub fn remove(&mut self, idx: isize) {
        unsafe {
            ffi::sq_remove(self.0, idx);
        }
    }
    ...
    pub fn set_top(&mut self, new_top: isize) {
        unsafe {
            ffi::sq_settop(self.0, new_top);
        }
    }

    pub fn reserve_stack(&self, n_size: isize) {
        unsafe {
            ffi::sq_reservestack(self.0, n_size);
        }
    }
    ...
    pub fn new_user_data<'a, T>(&mut self) -> &'a mut T {
        unsafe {
            mem::transmute(ffi::sq_newuserdata(self.0,
                                               mem::size_of::<T>() as ffi::SQUnsignedInteger))
        }
    }

    pub fn new_table(&mut self) {
        unsafe {
            ffi::sq_newtable(self.0);
        }
    }

    pub fn new_table_with_capacity(&mut self, capacity: usize) {
        unsafe {
            ffi::sq_newtableex(self.0, capacity as ffi::SQInteger);
        }
    }

    pub fn new_array(&mut self, size: usize) {
        unsafe {
            ffi::sq_newarray(self.0, size as ffi::SQInteger);
        }
    }
    ...
    pub fn set_params_check(&mut self, n_params_check: isize, type_mask: &str) -> Result<(), ()> {
        ...
        get_result(unsafe {
                       ffi::sq_setparamscheck(self.0,
                                              n_params_check as ffi::SQInteger,
                                              type_mask.as_ptr())
                   },
                   (),
                   ())
    ...
    pub fn bind_env(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_bindenv(self.0, idx) }, (), ())
    }

    pub fn push_str(&mut self, s: &str) {
        let s = CString::new(s).unwrap();
        unsafe {
            ffi::sq_pushstring(self.0, mem::transmute(s.as_ptr()), -1);
        }
    }

    pub fn push_float(&mut self, f: ffi::SQFloat) {
        unsafe {
            ffi::sq_pushfloat(self.0, f);
        }
    }

    pub fn push_integer(&mut self, n: ffi::SQInteger) {
        unsafe {
            ffi::sq_pushinteger(self.0, n);
        }
    }

    pub fn push_bool(&mut self, b: bool) {
        unsafe {
            ffi::sq_pushbool(self.0, b as ffi::SQBool);
        }
    }

    pub fn push_box<T>(&mut self, p: Box<T>) {
        unsafe {
            ffi::sq_pushuserpointer(self.0, mem::transmute(p));
        }
    }

    pub fn push_null(&mut self) {
        unsafe {
            ffi::sq_pushnull(self.0);
        }
    }
    
    pub fn get_type(&mut self, idx: ffi::SQInteger) -> Result<ObjectType, ()> {
        unsafe {
            match ffi::sq_gettype(self.0, idx) as isize {
                x if (x & ffi::_RT_NULL) != 0 => Ok(ObjectType::Null),
                x if (x & ffi::_RT_INTEGER) != 0 => Ok(ObjectType::Integer),
                x if (x & ffi::_RT_FLOAT) != 0 => Ok(ObjectType::Float),
                x if (x & ffi::_RT_BOOL) != 0 => Ok(ObjectType::Bool),
                x if (x & ffi::_RT_STRING) != 0 => Ok(ObjectType::String),
                x if (x & ffi::_RT_TABLE) != 0 => Ok(ObjectType::Table),
                x if (x & ffi::_RT_ARRAY) != 0 => Ok(ObjectType::Array),
                x if (x & ffi::_RT_USERDATA) != 0 => Ok(ObjectType::Userdata),
                x if (x & ffi::_RT_CLOSURE) != 0 => Ok(ObjectType::Closure),
                x if (x & ffi::_RT_NATIVECLOSURE) != 0 => Ok(ObjectType::NativeClosure),
                x if (x & ffi::_RT_GENERATOR) != 0 => Ok(ObjectType::Generator),
                x if (x & ffi::_RT_USERPOINTER) != 0 => Ok(ObjectType::UserPointer),
                x if (x & ffi::_RT_THREAD) != 0 => Ok(ObjectType::Thread),
                x if (x & ffi::_RT_CLASS) != 0 => Ok(ObjectType::Class),
                x if (x & ffi::_RT_INSTANCE) != 0 => Ok(ObjectType::Instance),
                x if (x & ffi::_RT_WEAKREF) != 0 => Ok(ObjectType::WeakRef),
                _ => Err(())
            }
        }
    }
    ...
    pub fn push_root_table(&mut self) {
        unsafe {
            ffi::sq_pushroottable(self.0);
        }
    }

    pub fn push_registry_table(&mut self) {
        unsafe {
            ffi::sq_pushregistrytable(self.0);
        }
    }

    pub fn push_const_table(&mut self) {
        unsafe {
            ffi::sq_pushconsttable(self.0);
        }
    }

    pub fn set_root_table(&mut self) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_setroottable(self.0) }, (), ())
    }

    pub fn set_const_table(&mut self) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_setconsttable(self.0) }, (), ())
    }

    pub fn new_slot(&mut self, idx: isize, bstatic: bool) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_newslot(self.0, idx, bstatic as ffi::SQBool) },
                   (),
                   ())
    }

    pub fn delete_slot(&mut self, idx: isize, push_val: bool) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_deleteslot(self.0, idx, push_val as ffi::SQBool) },
                   (),
                   ())
    }

    pub fn set(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_set(self.0, idx) }, (), ())
    }

    pub fn get(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_get(self.0, idx) }, (), ())
    }

    pub fn raw_set(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_rawset(self.0, idx) }, (), ())
    }

    pub fn raw_get(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_rawget(self.0, idx) }, (), ())
    }

    pub fn raw_delete_slot(&mut self, idx: isize, push_val: bool) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_rawdeleteslot(self.0, idx, push_val as ffi::SQBool) },
                   (),
                   ())
    }

    pub fn new_member(&mut self, idx: isize, bstatic: bool) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_newmember(self.0, idx, bstatic as ffi::SQBool) },
                   (),
                   ())
    }

    pub fn raw_new_member(&mut self, idx: isize, bstatic: bool) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_rawnewmember(self.0, idx, bstatic as ffi::SQBool) },
                   (),
                   ())
    }

    pub fn array_append(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_arrayappend(self.0, idx) }, (), ())
    }

    pub fn array_pop(&mut self, idx: isize, push_val: bool) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_arraypop(self.0, idx, push_val as ffi::SQBool) },
                   (),
                   ())
    }

    pub fn sq_arrayresize(&mut self, idx: isize, new_size: usize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_arraypop(self.0, idx, new_size) }, (), ())
    }

    pub fn array_reverse(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_arrayreverse(self.0, idx) }, (), ())
    }

    pub fn array_remove(&mut self, idx: isize, item_idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_arrayremove(self.0, idx, item_idx) },
                   (),
                   ())
    }

    pub fn array_insert(&mut self, idx: isize, dest_pos: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_arrayinsert(self.0, idx, dest_pos) },
                   (),
                   ())
    }

    pub fn set_delegate(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_setdelegate(self.0, idx) }, (), ())
    }

    pub fn get_delegate(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_getdelegate(self.0, idx) }, (), ())
    }

    // This seems like a conflicting name
    pub fn clone(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_clone(self.0, idx) }, (), ())
    }

    pub fn set_free_variable(&mut self, idx: isize, n_val: usize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_setfreevariable(self.0, idx, n_val) },
                   (),
                   ())
    }
    ...
    pub fn next(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_next(self.0, idx) }, (), ())
    }

    pub fn get_weak_ref_val(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_getweakrefval(self.0, idx) }, (), ())
    }

    pub fn clear(&mut self, idx: isize) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_clear(self.0, idx) }, (), ())
    }
    ...
    pub fn get_local(&mut self, level: usize, idx: usize) -> String {
        unsafe {
            from_utf8(CStr::from_ptr(ffi::sq_getlocal(self.0, level, idx)).to_bytes())
                .unwrap()
                .to_string()
        }
    }
    ...
    pub fn get_callee(&mut self) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_getcallee(self.0) }, (), ())
    }

    pub fn get_free_variable(&mut self, idx: isize, nval: usize) -> String {
        unsafe {
            from_utf8(CStr::from_ptr(ffi::sq_getfreevariable(self.0, idx, nval)).to_bytes())
                .unwrap()
                .to_string()
        }
    }
    ...
    pub fn throw_error(&mut self, error: &str) -> Result<(), ()> {
        ...
        get_result(unsafe { ffi::sq_throwerror(self.0, error.as_ptr()) },
                   (),
                   ())
    }

    pub fn throw_object(&mut self) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_throwobject(self.0) }, (), ())
    }
    ...
    pub fn collect_garbage(&mut self) -> isize {
        unsafe { ffi::sq_collectgarbage(self.0) }
    }

    pub fn resurrect_unreachable(&mut self) -> Result<(), ()> {
        get_result(unsafe { ffi::sq_resurrectunreachable(self.0) }, (), ())
    }
    ...
    pub fn stack_info(&self, level: isize) -> Result<StackInfo, ()> {
        ...
        let func_name = if si.funcname.is_null() {
            ...
            unsafe { from_utf8(CStr::from_ptr(si.funcname).to_bytes()).unwrap().to_string() }
        };

        let source = if si.source.is_null() {
            "".to_string()
        } else {
            unsafe { from_utf8(CStr::from_ptr(si.source).to_bytes()).unwrap().to_string() }
        };
